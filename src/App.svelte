<script>
  ...
  // 1. INITIALIZE FROM BUILD-TIME INJECTION
  // Note: __BUILD_CONFIG__ is provided by vite.config.js
  ...
  function handleKeydown(event) {
    ...
    if (event.key === 'Enter') {
      ...
      if (trimmed.startsWith(':')) {
        ...
        // Dynamic edits (persists until refresh/rebuild)
...
<main class="theme-{currentTheme}">
  ...
  <div class="terminal glass">
    <div class="header">
        <div class="comment"># Static Build v5.5 | Mode: {currentTheme}</div>
    </div>
    ...
    <div class="output-area">
      {#if showHelp}
        <div class="special-view">
          ...
          <div class="help-row"><span class="cmd">:add</span> <span class="desc">name url key? (Temporary)</span></div>
      ...
      {:else}
        {#each filteredSites as site}
          <div class="row-container" 
               ...
               on:click={() => window.open(site.url, '_blank')}>
            <div class="row">
    ...
    <div class="input-container-wrapper">
      ...
      <div class="input-line">
        ...
        <div class="input-wrapper">
          <input bind:value={query} on:keydown={handleKeydown} placeholder="Search or :command..." autofocus spellcheck="false" />
        </div>
      </div>
    </div>
  </div>
