<script>
  ...
  const changelog = [
    { v: "5.9", f: "Weather caching (15m) for instantaneous dashboard loading." },
    { v: "5.8", f: "Fixed :add/:del logic & expanded input field width." },
    { v: "5.7", f: "Dashboard: Added real-time clock & live weather fetch." }
  ];
  ...
  onMount(() => {
    ...
    if (cached) {
      ...
      if (Date.now() - timestamp < CACHE_TTL) {
        weather = data; // Instant load from cache
      }
    }
    ...
    fetch('https://wttr.in/?format=j1')
      ...
      .catch(() => {
        if (weather.temp === '--') weather = { temp: '!!', desc: 'Offline' };
      });
  ...
  // --- Search & Keydown Logic (Restored from v5.8) ---
