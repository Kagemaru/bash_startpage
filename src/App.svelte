<script>
  ...
  const changelog = [
    { v: "5.8", f: "Fixed :add/:del logic & expanded input field to full width." },
    ...
    { v: "4.0", f: "Theme: Catppuccin Mocha theme & Glassmorphism." },
    { v: "2.8", f: "Logic: Vim-style ':' prefix for system commands." },
  ...
  onMount(() => {
    ...
    fetch('https://wttr.in/?format=j1')
      ...
      .then(data => {
        ...
        weather = { temp: current.temp_C + '°C', desc: current.weatherDesc[0].value };
      })
      .catch(() => weather = { temp: '!!', desc: 'Weather Error' });
  ...
  const setError = (msg) => { errorMsg = msg; setTimeout(() => errorMsg = '', 3000); };
    ...
    if (event.key === 'Enter') {
      ...
      if (trimmed.startsWith(':')) {
        ...
        if (cmd === 'add' && parts.length >= 3) {
          sites = [...sites, { 
            name: parts[1], 
            url: parts[2].includes('.') ? (parts[2].startsWith('http') ? parts[2] : `https://${parts[2]}`) : parts[2], 
        ...
        if (cmd === 'tag' && parts.length >= 3) {
          ...
          sites = sites.map(s => (s.name.toLowerCase() === id || s.shortcuts.includes(id)) 
...
<main class="theme-{currentTheme}">
  ...
  <div class="terminal glass">
    <div class="header">
        <div class="comment"># Dashboard | Mode: {currentTheme}</div>
        <div class="dashboard">
    ...
    <div class="input-container-wrapper">
      {#if errorMsg}<div class="error-toast">{errorMsg}</div>{/if}
      {#if suggestions.length > 0}
...
<style>
  ...
  .tooltip { position: fixed; pointer-events: none; padding: 6px 12px; border-radius: 8px; font-size: 0.75rem; font-weight: 600; color: var(--text); border: 1px solid rgba(255,255,255,0.2); box-shadow: 0 10px 20px rgba(0,0,0,0.4); z-index: 1000; }

  /* FIX: Ensure input spans full width */
